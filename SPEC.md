...
- **D9 — Decide mode v1 (intelligence floor + cost×speed).** User-agnostic AA Index floor (default 50 on enter), pick-eligible = Index ≥ floor + scorable cost/speed, cost×speed Pareto shortlist of 3 with cheap↔fast bias, shareable URL (`decide`/`floor`/`bias`/`anchor`), DecideResponse JSON export. While Decide is on, classic value-score optimum is suppressed (single ranking authority). No product AI / suite priors in v1. Authority: Forgejo SPEC #137, map #128, ralplan B′ `.omx/plans/prd-decide-mode-v1.md`.
