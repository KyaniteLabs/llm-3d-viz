...
5b. **Console + readout + tooltip** — `src/ui/console.ts`: three weight sliders + five preset chips + model readout panel (reasoner models with a multi-minute TTFT carry the "incl. thinking time (long-prompt median)" caveat, gated on name/metadata via `src/lib/format.ts` `isReasoningModel`, C15) + cursor-anchored HTML tooltip with click-to-pin + incomplete-data section listing each excluded model with its missing-axis reason label (C10).
...
- **C15** TTFT readout label: "TTFT incl. reasoning (long prompt)".
  - **CORRECTED 2026-08-02 (PR #31 review, Forgejo #28):** the shipped caveat is `"incl. thinking time (long-prompt median)"` — a footnote *under* the TTFT value, not the value's label — and it is gated on the model being a reasoning/thinking-effort model **and** having a multi-minute TTFT (`src/lib/format.ts`: `isReasoningModel` + `ttftCaveat`). A bare ms threshold alone is wrong: a slow NON-reasoning model is just slow, not "thinking", so the caveat must not attach to it. Original C15 wording preserved as the historical decision; step 5b states the current contract. (History preserved per instruction; this note appended, not retroactive.)
